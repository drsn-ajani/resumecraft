...
app.use((req, res, next) => {
  console.log(`[REQ] ${new Date().toISOString()} ${req.method} ${req.originalUrl} from ${req.ip} origin=${req.headers.origin || "none"}`);
  res.on("finish", () => {
...
mongoose
  ...
  .then(() =>
    console.log(
      `Connected to MongoDB Atlas host=${mongoose.connection.host} db=${mongoose.connection.name}`
    )
  )
...
app.post("/users", async (req, res) => {
  try {
    ...
    console.log(`[DB] user saved _id=${savedUser._id} db=${mongoose.connection.name} host=${mongoose.connection.host}`);
